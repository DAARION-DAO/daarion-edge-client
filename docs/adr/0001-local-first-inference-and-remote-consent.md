...
The Phase 1A candidate routes status, discovery, preparation, smoke and chat through one bounded `InferenceService`; production composition constructs only the validated loopback Ollama provider and revalidates the reported endpoint at each service boundary. The service owns a separate five-second probe deadline for health and installed-model inventory, while chat and model preparation retain their own longer-lived semantics. Unit and loopback-fixture tests cover stalled probe headers/body, endpoint rejection, unique canonical mapping, bounded stream framing, malformed/post-terminal provider records, queue/stream deadline and cancellation races, duplicate IDs and sole-terminal-event enforcement. The mounted UI uses one typed command/event adapter and main-window shell authority was removed.
